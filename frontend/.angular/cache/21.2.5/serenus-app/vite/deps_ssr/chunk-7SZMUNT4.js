...
function wrapIntoObservable(value) {
  ...
  if (isPromise(value)) {
    return (0, import_rxjs3.from)(Promise.resolve(value));
  }
...
function createUrlTreeFromSegmentGroup(relativeTo, commands, queryParams, fragment, urlSerializer) {
  ...
  if (commands.length === 0) {
    return tree(root2, root2, root2, queryParams, fragment, urlSerializer);
  }
  const nav = computeNavigation(commands);
  if (nav.toRoot()) {
    return tree(root2, root2, new UrlSegmentGroup([], {}), queryParams, fragment, urlSerializer);
  }
  const position = findStartingPositionForTargetGroup(nav, root2, relativeTo);
  const newSegmentGroup = position.processChildren ? updateSegmentGroupChildren(position.segmentGroup, position.index, nav.commands) : updateSegmentGroup(position.segmentGroup, position.index, nav.commands);
  return tree(root2, position.segmentGroup, newSegmentGroup, queryParams, fragment, urlSerializer);
}
...
function createEmptyState(rootComponent, injector) {
  ...
  const emptyUrl = new import_rxjs3.BehaviorSubject([new UrlSegment("", {})]);
  const emptyParams = new import_rxjs3.BehaviorSubject({});
  const emptyData = new import_rxjs3.BehaviorSubject({});
  const emptyQueryParams = new import_rxjs3.BehaviorSubject({});
  const fragment = new import_rxjs3.BehaviorSubject("");
...
var ActivatedRoute = class {
  ...
  constructor(urlSubject, paramsSubject, queryParamsSubject, fragmentSubject, dataSubject, outlet, component, futureSnapshot) {
    ...
    this.title = this.dataSubject?.pipe((0, import_operators.map)((d) => d[RouteTitleKey])) ?? (0, import_rxjs3.of)(void 0);
    this.url = urlSubject;
...
var ActivatedRouteSnapshot = class {
  ...
  constructor(url, params, queryParams, fragment, data, outlet, component, routeConfig, resolve2, environmentInjector) {
    this.url = url;
    this.params = params;
    this.queryParams = queryParams;
    this.fragment = fragment;
...
var RoutedComponentInputBinder = class _RoutedComponentInputBinder {
  ...
  subscribeToRouteData(outlet) {
    ...
    const dataSubscription = (0, import_rxjs3.combineLatest)([activatedRoute.queryParams, activatedRoute.params, activatedRoute.data]).pipe((0, import_operators.switchMap)(([queryParams, params, data], index2) => {
...
function createActivatedRoute(c) {
  return new ActivatedRoute(new import_rxjs3.BehaviorSubject(c.url), new import_rxjs3.BehaviorSubject(c.params), new import_rxjs3.BehaviorSubject(c.queryParams), new import_rxjs3.BehaviorSubject(c.fragment), new import_rxjs3.BehaviorSubject(c.data), c.outlet, c.component, c);
}
...
function isEmptyError(e) {
  return e instanceof import_rxjs3.EmptyError || e?.name === "EmptyError";
}
var INITIAL_VALUE = /* @__PURE__ */ Symbol("INITIAL_VALUE");
function prioritizedGuardValue() {
  return (0, import_operators.switchMap)((obs) => {
    return (0, import_rxjs3.combineLatest)(obs.map((o) => o.pipe((0, import_operators.take)(1), (0, import_operators.startWith)(INITIAL_VALUE)))).pipe((0, import_operators.map)((results) => {
...
function abortSignalToObservable(signal2) {
  if (signal2.aborted) {
    return (0, import_rxjs3.of)(void 0).pipe((0, import_operators.take)(1));
  }
...
function checkGuards(forwardEvent) {
  return (0, import_operators.mergeMap)((t) => {
    ...
    if (canDeactivateChecks.length === 0 && canActivateChecks.length === 0) {
      return (0, import_rxjs3.of)(__spreadProps(__spreadValues({}, t), {
        guardsResult: true
      }));
    }
    return runCanDeactivateChecks(canDeactivateChecks, targetSnapshot, currentSnapshot).pipe((0, import_operators.mergeMap)((canDeactivate) => {
      return canDeactivate && isBoolean(canDeactivate) ? runCanActivateChecks(targetSnapshot, canActivateChecks, forwardEvent) : (0, import_rxjs3.of)(canDeactivate);
...
function runCanDeactivateChecks(checks, futureRSS, currRSS) {
  return (0, import_rxjs3.from)(checks).pipe((0, import_operators.mergeMap)((check) => runCanDeactivate(check.component, check.route, currRSS, futureRSS)), (0, import_operators.first)((result2) => {
    return result2 !== true;
  }, true));
}
function runCanActivateChecks(futureSnapshot, checks, forwardEvent) {
  return (0, import_rxjs3.from)(checks).pipe((0, import_operators.concatMap)((check) => {
    return (0, import_rxjs3.concat)(fireChildActivationStart(check.route.parent, forwardEvent), fireActivationStart(check.route, forwardEvent), runCanActivateChild(futureSnapshot, check.path), runCanActivate(futureSnapshot, check.route));
...
function runCanActivate(futureRSS, futureARS) {
  ...
  if (!canActivate || canActivate.length === 0) return (0, import_rxjs3.of)(true);
  ...
  return (0, import_rxjs3.of)(canActivateObservables).pipe(prioritizedGuardValue());
}
function runCanActivateChild(futureRSS, path2) {
  ...
  const canActivateChildGuardsMapped = canActivateChildGuards.map((d) => {
    return (0, import_rxjs3.defer)(() => {
      ...
      return (0, import_rxjs3.of)(guardsMapped).pipe(prioritizedGuardValue());
    });
  });
  return (0, import_rxjs3.of)(canActivateChildGuardsMapped).pipe(prioritizedGuardValue());
}
function runCanDeactivate(component, currARS, currRSS, futureRSS) {
  const canDeactivate = currARS && currARS.routeConfig ? currARS.routeConfig.canDeactivate : null;
  if (!canDeactivate || canDeactivate.length === 0) return (0, import_rxjs3.of)(true);
  ...
  return (0, import_rxjs3.of)(canDeactivateObservables).pipe(prioritizedGuardValue());
}
function runCanLoadGuards(injector, route, segments, urlSerializer, abortSignal) {
  ...
  return (0, import_rxjs3.of)(canLoadObservables).pipe(prioritizedGuardValue(), redirectIfUrlTree(urlSerializer));
}
function redirectIfUrlTree(urlSerializer) {
  return (0, import_rxjs3.pipe)((0, import_operators.tap)((result2) => {
...
function runCanMatchGuards(injector, route, segments, urlSerializer, currentSnapshot, abortSignal) {
  ...
  if (!canMatch || canMatch.length === 0) return (0, import_rxjs3.of)(true);
  ...
  return (0, import_rxjs3.of)(canMatchObservables).pipe(prioritizedGuardValue(), redirectIfUrlTree(urlSerializer));
}
...
function matchWithChecks(segmentGroup, route, segments, injector, urlSerializer, createSnapshot, abortSignal) {
  ...
  if (!result2.matched) {
    return (0, import_rxjs3.of)(result2);
  }
  const currentSnapshot = createPreMatchRouteSnapshot(createSnapshot(result2));
  injector = getOrCreateRouteInjectorIfNeeded(route, injector);
  return runCanMatchGuards(injector, route, segments, urlSerializer, currentSnapshot, abortSignal).pipe((0, import_operators.map)((v) => v === true ? result2 : __spreadValues({}, noMatch)));
}
...
This is currently a dev mode only error but will become a call stack size exceeded error in production in a future major version.`);
  ...
  async matchSegmentAgainstRoute(injector, rawSegment, route, segments, outlet, parentRoute) {
    ...
    const createSnapshot = (result3) => this.createSnapshot(injector, route, result3.consumedSegments, result3.parameters, parentRoute);
    const result2 = await firstValueFrom(matchWithChecks(rawSegment, route, segments, injector, this.urlSerializer, createSnapshot, this.abortSignal));
    if (route.path === "**") {
      rawSegment.children = {};
    }
    if (!result2?.matched) {
...
function resolveData(paramsInheritanceStrategy) {
  return (0, import_operators.mergeMap)((t) => {
    ...
    return (0, import_rxjs3.from)(routesNeedingDataUpdates).pipe((0, import_operators.concatMap)((route) => {
    ...
    }), (0, import_operators.tap)(() => routesProcessed++), (0, import_operators.takeLast)(1), (0, import_operators.mergeMap)((_) => routesProcessed === routesNeedingDataUpdates.size ? (0, import_rxjs3.of)(t) : import_rxjs3.EMPTY));
  });
}
...
function runResolve(futureARS, futureRSS, paramsInheritanceStrategy) {
  ...
  return (0, import_rxjs3.defer)(() => {
    ...
    return resolveNode(resolve2, futureARS, futureRSS).pipe((0, import_operators.map)((resolvedData) => {
...
function resolveNode(resolve2, futureARS, futureRSS) {
  const keys = getDataKeys(resolve2);
  if (keys.length === 0) {
    return (0, import_rxjs3.of)({});
  }
  const data = {};
  return (0, import_rxjs3.from)(keys).pipe((0, import_operators.mergeMap)((key) => getResolver(resolve2[key], futureARS, futureRSS).pipe((0, import_operators.first)(), (0, import_operators.tap)((value) => {
    if (value instanceof RedirectCommand) {
      throw redirectingNavigationError(new DefaultUrlSerializer(), value);
    }
    data[key] = value;
  }))), (0, import_operators.takeLast)(1), (0, import_operators.map)(() => data), (0, import_operators.catchError)((e) => isEmptyError(e) ? import_rxjs3.EMPTY : (0, import_rxjs3.throwError)(e)));
}
...
function switchTap(next) {
  return (0, import_operators.switchMap)((v) => {
    ...
    if (nextResult) {
      return (0, import_rxjs3.from)(nextResult).pipe((0, import_operators.map)(() => v));
    }
    return (0, import_rxjs3.of)(v);
  });
}
...
var RouterConfigLoader = class _RouterConfigLoader {
  ...
  loadChildren(parentInjector, route) {
    ...
    const loader = (async () => {
      try {
        const result2 = await loadChildren(route, this.compiler, parentInjector, this.onLoadEndListener);
        route._loadedRoutes = result2.routes;
        route._loadedInjector = result2.injector;
        route._loadedNgModuleFactory = result2.factory;
        return result2;
      } finally {
...
var NavigationTransitions = class _NavigationTransitions {
  ...
  transitionAbortWithErrorSubject = new import_rxjs3.Subject();
  ...
  transitions;
  afterPreactivation = () => (0, import_rxjs3.of)(void 0);
  ...
  setupNavigations(router) {
    this.transitions = new import_rxjs3.BehaviorSubject(null);
    return this.transitions.pipe((0, import_operators.filter)((t) => t !== null), (0, import_operators.switchMap)((overallTransitionState) => {
      ...
      return (0, import_rxjs3.of)(overallTransitionState).pipe((0, import_operators.switchMap)((t) => {
        ...
        if (this.urlHandlingStrategy.shouldProcessUrl(t.rawUrl)) {
          return (0, import_rxjs3.of)(t).pipe((0, import_operators.switchMap)((t2) => {
          ...
          }), (0, import_operators.switchMap)((value) => (0, import_rxjs3.from)(overallTransitionState.routesRecognizeHandler.deferredHandle ?? (0, import_rxjs3.of)(void 0)).pipe((0, import_operators.map)(() => value))), (0, import_operators.tap)(() => {
          ...
          return (0, import_rxjs3.of)(overallTransitionState);
        } else {
        ...
        return (0, import_rxjs3.of)(t).pipe(resolveData(this.paramsInheritanceStrategy), (0, import_operators.tap)({
          next: () => {
            dataResolved = true;
        ...
        return loaders.length === 0 ? (0, import_rxjs3.of)(t) : (0, import_rxjs3.from)(Promise.all(loaders).then(() => t));
        ...
        return viewTransitionStarted ? (0, import_rxjs3.from)(viewTransitionStarted).pipe((0, import_operators.map)(() => overallTransitionState)) : (0, import_rxjs3.of)(overallTransitionState);
        ...
        return deferred ? (0, import_rxjs3.from)(deferred.then(() => t)) : (0, import_rxjs3.of)(t);
...
function afterNextNavigation(router, action) {
  ...
  }), (0, import_operators.filter)((result2) => result2 !== 2), (0, import_operators.take)(1)).subscribe(() => {
    action();
  });
}
var Router = class _Router {
  ...
  eventsSubscription = new import_rxjs3.Subscription();
...
var ReactiveRouterState = class _ReactiveRouterState {
  ...
  updateState() {
    ...
    this.path.set(this.serializer.serialize(new UrlTree(root2)));
  }
...
var RouterLink = class _RouterLink {
  ...
  isAnchorElement;
  onChanges = new import_rxjs4.Subject();
...
var RouterLinkActive = class _RouterLinkActive {
  ...
  ngAfterContentInit() {
    (0, import_rxjs4.of)(this.links.changes, (0, import_rxjs4.of)(null)).pipe((0, import_operators2.mergeAll)()).subscribe((_) => {
      this.update();
  ...
  subscribeToEachLinkOnChanges() {
    ...
    this.linkInputChangesSubscription = (0, import_rxjs4.from)(allLinkChanges).pipe((0, import_operators2.mergeAll)()).subscribe((link2) => {
...
var PreloadAllModules = class _PreloadAllModules {
  preload(route, fn) {
    return fn().pipe((0, import_operators2.catchError)(() => (0, import_rxjs4.of)(null)));
  }
...
var RouterPreloader = class _RouterPreloader {
  ...
  processRoutes(injector, routes) {
    ...
    return (0, import_rxjs4.from)(res).pipe((0, import_operators2.mergeAll)());
  }
  preloadConfig(injector, route) {
    return this.preloadingStrategy.preload(route, () => {
      ...
      if (route.loadChildren && route.canLoad === void 0) {
        loadedChildren$ = (0, import_rxjs4.from)(this.loader.loadChildren(injector, route));
      } else {
        loadedChildren$ = (0, import_rxjs4.of)(null);
      }
      const recursiveLoadChildren$ = loadedChildren$.pipe((0, import_operators2.mergeMap)((config) => {
        if (config === null) {
          return (0, import_rxjs4.of)(void 0);
        }
      ...
      if (route.loadComponent && !route._loadedComponent) {
        ...
        return (0, import_rxjs4.from)([recursiveLoadChildren$, loadComponent$]).pipe((0, import_operators2.mergeAll)());
      } else {
...
var NavigationStateManager = class _NavigationStateManager extends StateManager {
  ...
  currentNavigation = {};
  nonRouterCurrentEntryChangeSubject = new import_rxjs4.Subject();
  ...
  registerNonRouterCurrentEntryChangeListener(listener) {
    ...
    this.nonRouterEntryChangeListener = this.nonRouterCurrentEntryChangeSubject.subscribe(({
      ...
      listener(path2, state, "popstate", !this.precommitHandlerSupported ? {
        replaceUrl: true
      } : {});
    });
  ...
  async handleRouterEvent(e, transition) {
    ...
    if (e instanceof NavigationStart) {
      ...
      transition.routesRecognizeHandler.deferredHandle = new Promise(async (resolve2) => {
        if (this.urlUpdateStrategy === "eager") {
          try {
            this.maybeCreateNavigationForTransition(transition);
      ...
      transition.beforeActivateHandler.deferredHandle = new Promise(async (resolve2) => {
  ...
  maybeCreateNavigationForTransition(transition) {
    ...
    const path2 = this.createBrowserPath(transition);
    this.navigate(path2, transition);
  }
  navigate(internalPath, transition) {
    ...
    const history = this.location.isCurrentPathEqualTo(path2) || transition.extras.replaceUrl || transition.extras.skipLocationChange ? "replace" : "push";
    handleResultRejections(this.navigation.navigate(path2, {
      state,
      history,
      info
    }));
  ...
  handleNavigateEventTriggeredOutsideRouterAPIs(event) {
    const path2 = event.destination.url.substring(this.appRootURL.length - 1);
...
function handleResultRejections(result2) {
  result2.finished?.catch(() => {
  });
  result2.committed?.catch(() => {
  });
  return result2;
}
function rootRoute() {
...
var BOOTSTRAP_DONE = new InjectionToken(typeof ngDevMode === "undefined" || ngDevMode ? "bootstrap done indicator" : "", {
  factory: () => {
    return new import_rxjs4.Subject();
  }
});
    ...
    return locationInitialized.then(() => {
      return new Promise((resolve2) => {
        ...
        injector.get(NavigationTransitions).afterPreactivation = () => {
          resolve2(true);
          return bootstrapDone.closed ? (0, import_rxjs4.of)(void 0) : bootstrapDone;
        };
        router.initialNavigation();
      });
    });
...
function requireInput() {
  ...
  class Input2 {
    ...
    origin(line, column, endLine, endColumn) {
      ...
      let from3 = consumer.originalPositionFor({ column, line });
              ...
              if (typeof plugin[event] === "object") {
                for (let filter4 in plugin[event]) {
                  if (filter4 === "*") {
                    ...
                    add(
                      plugin,
                      event + "-" + filter4.toLowerCase(),
                      plugin[event][filter4]
                    );
                  }
                }
...
//# sourceMappingURL=chunk-7SZMUNT4.js.map
